...
// qrDataURI renders content as a PNG QR code and returns it as a base64
// data: URI so it can be embedded inline (no external requests, fully offline).
...
// WritePaperWallet renders a self-contained, fully-offline HTML paper wallet to
// path. Every asset (CSS, QR images) is inlined; the page makes zero network
// requests and is optimised for printing on A4/Letter in black and white.
func WritePaperWallet(path, network, version, address, seed, txHex string) error {
	...
	var err error
	if d.AddressQR, err = qrDataURI(address, 280); err != nil {
		return fmt.Errorf("address QR: %w", err)
	}
	if d.SeedQR, err = qrDataURI(seed, 360); err != nil {
		return fmt.Errorf("seed QR: %w", err)
	}
	if d.HasTx {
		if d.TxQR, err = qrDataURI(txHex, 280); err != nil {
			return fmt.Errorf("tx QR: %w", err)
		}
	}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* engraved certificate frame, sits in the margin, clear of content */
  .sheet::before {
  ...
  /* The secret vault panel: the signature element */
  .vault {
    margin-top: 16px;
  ...
  @media print {
    ...
    /* Fill the @page box; do not stack the sheet's screen padding on top of the
       page margin (that pushed content onto a second page). */
...
<main class="sheet">
  ...
  <footer>
    <span>Generated offline by DERO ColdWallet, {{.Version}}</span>
    <span>Non-commercial, DERO Research License</span>
  </footer>
</main>
</body>
</html>
